...
from app.services.break_calculator import (
    calculate_break_time,
    calculate_actual_breaks,
    get_excluded_procedure_codes,
    calculate_total_hours_with_drive_time,
    get_break_configuration,
    analyze_lunch_compliance,
)
...
def process_uploaded_file(file_path: str, file_id: str, user_id: int, progress_tracker=None) -> Dict[str, Any]:
    ...
    try:
        ...
        #validate scheduling data structure StartDateTime	EndDateTime

        required_scheduling_columns = ['Principal1', 'Principal1Name', 'StartDateTime', 'EndDateTime', 'Date', 'SegmentHours', 'CodeName']
        missing_scheduling_columns = [col for col in required_scheduling_columns if col not in df.columns]
        if missing_scheduling_columns:
            raise ValueError(f"Missing required scheduling columns: {', '.join(missing_scheduling_columns)}. Please ensure the scheduling data is included in the uploaded file.")
        
        update_progress(12, "Renaming scheduling columns to standard format...")
        
        # Rename scheduling columns to standard processing format
        df = df.rename(columns={
            'Principal1': 'ProviderId',
            'Date': 'DateOfService',
            'SegmentHours': 'TimeWorkedInHours',
            'CodeName': 'ProcedureCode'
        })
        
        # Split Principal1Name (full name) into ProviderFirstName and ProviderLastName
        if 'Principal1Name' in df.columns:
            # Split on the first space - everything before is FirstName, everything after is LastName
            name_split = df['Principal1Name'].str.split(' ', n=1, expand=True)
            df['ProviderFirstName'] = name_split[0] if 0 in name_split.columns else ''
            df['ProviderLastName'] = name_split[1] if 1 in name_split.columns else ''
            # Fill NaN values with empty string
            df['ProviderFirstName'] = df['ProviderFirstName'].fillna('')
            df['ProviderLastName'] = df['ProviderLastName'].fillna('')
        ...
        if df['TimeWorkedInHours'].isnull().any():
            raise ValueError("TimeWorkedInHours column contains empty values. All entries must have hours worked.")
                # Check for IsVoid column and remove rows where IsVoid is True
        if 'IsVoid' in df.columns:
            # Convert IsVoid to boolean, handling various formats
            df['IsVoid'] = df['IsVoid'].astype(str).str.upper().isin(['TRUE', '1', 'YES', 'Y'])
            
            # Count voided entries for reporting
            voided_count = df['IsVoid'].sum()
            
            # Remove voided entries
            df = df[~df['IsVoid']].copy()
            
            # Drop the IsVoid column as it's no longer needed
            df = df.drop('IsVoid', axis=1)
            
            if voided_count > 0:
                update_progress(18, f"Removed {voided_count} voided entries...")
        ...
        df['ProviderFullName'] = df['ProviderFirstName'].astype(str) + ' ' + df['ProviderLastName'].astype(str)
        ...
        if not df['DateOfService'].isna().all():
            df['DateOfService'] = df['DateOfService'].apply(lambda x: x.date() if pd.notna(x) else None)
        
        # Convert back to datetime for processing but maintain date-only format
        df['DateOfService_datetime'] = pd.to_datetime(df['DateOfService'])
        
        # Calculate date range
        date_min_val = df['DateOfService_datetime'].min()
        date_max_val = df['DateOfService_datetime'].max()
        date_min = date_min_val.strftime('%m/%d/%Y') if pd.notna(date_min_val) else 'N/A'
        date_max = date_max_val.strftime('%m/%d/%Y') if pd.notna(date_max_val) else 'N/A'
        ...
        for idx, (_, row) in enumerate(daily_hours.iterrows()):
            ...
            # Analyze lunch timing compliance if timing columns exist
            lunch_timing_info = analyze_lunch_compliance(provider_day_data, excluded_codes)
            ...
            record = {
                ...
                'BreakCompliance': 'Compliant' if int(total_break_minutes) == int(row['RequiredBreakMinutes']) else 'Non-Compliant',
                'BreakDeficitMinutes': max(0, row['RequiredBreakMinutes'] - total_break_minutes)
            }

            # Merge lunch timing info into record
            record.update({
                'LunchTimingRequired': lunch_timing_info.get('LunchTimingRequired', False),
                'LunchTimingCompliant': lunch_timing_info.get('LunchTimingCompliant', None),
                'LunchTimingStatus': lunch_timing_info.get('LunchTimingStatus', ''),
                # 'FirstWorkStart': lunch_timing_info.get('FirstWorkStart'),
                # 'FirstLunchStart': lunch_timing_info.get('FirstLunchStart'),
                # 'LunchDeadline': lunch_timing_info.get('LunchDeadline'),
                # 'CumulativeWorkBeforeLunchHours': lunch_timing_info.get('CumulativeWorkBeforeLunchHours'),
                # 'LunchFirstDurationMinutes': lunch_timing_info.get('LunchFirstDurationMinutes', 0),
            })

            actual_breaks_data.append(record)
        ...
        # Add lunch timing compliance for provider-date summary
        if not daily_hours.empty:
            lunch_compliance_map = daily_hours.groupby(['ProviderId', 'ProviderFullName', 'DateOfService'])['LunchTimingCompliant'].first().reset_index()
            provider_date_summary = provider_date_summary.merge(
            lunch_compliance_map[['ProviderId', 'ProviderFullName', 'DateOfService', 'LunchTimingCompliant']], 
            on=['ProviderId', 'ProviderFullName', 'DateOfService'], 
            how='left'
            )
            provider_date_summary['LunchTimingCompliance'] = provider_date_summary['LunchTimingCompliant'].apply(
            lambda x: 'Compliant' if x is True else ('Non-Compliant' if x is False else 'Unknown')
            )
        else:
            provider_date_summary['LunchTimingCompliance'] = 'Unknown'
        ...
        # Derive lunch timing compliance summary metrics
        if not daily_hours.empty:
            # Treat None (unknown) as non-compliant-unknown category; we won't include in compliant count
            daily_hours['LunchTimingCompliant_bool'] = daily_hours['LunchTimingCompliant'].apply(lambda v: True if v is True else False)
            provider_lunch_timing = daily_hours.groupby(['ProviderId', 'ProviderFullName'])['LunchTimingCompliant_bool'].agg(['sum', 'count']).reset_index()
            provider_lunch_timing = provider_lunch_timing.rename(columns={'sum': 'DaysLunchTimingCompliant', 'count': 'DaysWithRecords'})
            provider_summary = provider_summary.merge(provider_lunch_timing, on=['ProviderId', 'ProviderFullName'], how='left')
        else:
            provider_summary['DaysLunchTimingCompliant'] = 0
            provider_summary['DaysWithRecords'] = 0
        ...
        provider_summary['BreakTimeCompliance'] = provider_summary['BreakDeficitMinutes'].apply(
            lambda x: 'Compliant' if x == 0 else 'Non-Compliant'
        )
        provider_summary['LunchTimeCompliance'] = provider_summary.apply(
            lambda row: 'Compliant' if row['DaysLunchTimingCompliant'] == row['DaysWithRecords'] and row['DaysWithRecords'] > 0 else 'Non-Compliant',
            axis=1
        )
        
        daily_hours.drop(columns=['LunchTimingCompliant_bool'], inplace=True, errors='ignore')
        ...
        # Add lunch timing audit issues for non-compliant days
        try:
            for rec in (daily_hours.to_dict('records') if not daily_hours.empty else []):
                if rec.get('LunchTimingRequired') and rec.get('LunchTimingCompliant') is False:
                    audit_results.append({
                        'Type': 'CA Lunch Timing Non-Compliance',
                        'ProviderId': rec['ProviderId'],
                        'ProviderName': rec['ProviderFullName'],
                        'DateOfService': rec['DateOfService'],
                        'Issue': f"Lunch not compliant: {rec.get('LunchTimingStatus','')} (First work start {rec.get('FirstWorkStart')}, deadline {rec.get('LunchDeadline')}, lunch start {rec.get('FirstLunchStart')})",
                        'Severity': 'High'
                    })
        except Exception:
            pass
