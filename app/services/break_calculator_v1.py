...
def calculate_break_time(hours_worked: float, break_config: Optional[Dict[str, Any]] = None) -> int:
    ...
    Implements California labor law break requirements with mandatory 10-minute breaks.
    
    Business Rules:
    - 3.5 to <4 hours (threshold_1 to <4): One mandatory 10-minute break
    - 5+ hours continuous session: One mandatory 10-minute break (lunch eligible separately)
    - 6+ hours (threshold_2): Two mandatory 10-minute breaks (lunch optional/waivable with form)
    - 10+ hours (threshold_3): Three mandatory 10-minute breaks
    - 14+ hours (threshold_4): Four mandatory 10-minute breaks
    - 10-minute breaks are MANDATORY and cannot be waived under any condition
    - Breaks must be scheduled between or during work sessions
    
    Args:
        hours_worked: Number of hours worked (continuous session or total day time)
        break_config: Pre-fetched break configuration (for performance optimization)
        
    Returns:
        Required break time in minutes (10-minute breaks only, lunch calculated separately)
    """
    ...
    # Rule 1: For work time >= 14 hours (threshold_4)
    # Four mandatory 10-minute breaks (40 minutes total)
    if hours_worked >= threshold_4:
        return duration_4
    
    # Rule 2: For work time >= 10 hours (threshold_3) and < 14 hours
    # Three mandatory 10-minute breaks (30 minutes total)
    if hours_worked >= threshold_3:
        return duration_3
    
    # Rule 3: For work time >= 6 hours (threshold_2) and < 10 hours
    # Two mandatory 10-minute breaks (20 minutes total)
    # Lunch is optional and may be waived with Meal Break Waiver Form
    if hours_worked >= threshold_2:
        return duration_2
    
    # Rule 4: For work time >= 3.5 hours (threshold_1) and < 6 hours
    # One mandatory 10-minute break (10 minutes total)
    # For 5+ hour sessions, staff is eligible for lunch break (calculated separately)
    # Lunch can be waived with Meal Break Waiver Form if work time <= 6 hours
    if hours_worked >= threshold_1:
        return duration_1
    
    # Rule 5: For work time < 3.5 hours (threshold_1)
    # No mandatory breaks required
    return 0
